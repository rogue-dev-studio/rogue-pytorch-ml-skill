...
PyTorch deep learning framework. Use when the project needs this capability or the user / team manifest asks for it.

## When to use

- `TEAM.yaml` includes `pytorch-ml` or user asks by name
...
2. **Confirm tooling** - Verify `pytorch-ml` runtime/CLI/MCP is available; document install if missing.
